...
const LevinTransferReceipt = ({ levinResponse, onRepeat, sender }) => {
  ...
  const fields = [
    ...
    ["Account Number", levinResponse?.beneficiary?.account_number || "---"],
    ["Amount", `₹${levinResponse?.data?.AmountRequested || "---"}`],
    ["Transfer Mode", levinResponse?.transferMode || "---"],
    ["Purpose", levinResponse?.purpose || "N/A"],
    ["UTR", levinResponse?.data?.utr || "---"],
  ];

  const handlePrint = () => {
    const printWin = window.open("", "_blank");

    const transactionDate = levinResponse?.response?.data?.transactionDate
      ? new Date(levinResponse.response.data.transactionDate)
      : new Date();
    ...
    const values = [
      levinResponse?.response?.data?.mobileNumber || "---",
      levinResponse?.sender_name || "---",
      ...
      levinResponse?.response?.data?.transferMode || "---",
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .status { text-align: center; font-weight: bold; margin: 15px 0; color: ${
            levinResponse?.response?.status === "FAILED" ? "red" : "green"
          }; }
      ...
      <body>
        ...
        <div class="status">
          Status: ${
            levinResponse?.response?.status === "FAILED" ? "FAILED" : "SUCCESS"
          } | ${levinResponse?.response?.message || "Transaction Successful"}
        </div>
        <table>
          <thead>
            <tr>
...
export default LevinTransferReceipt;
